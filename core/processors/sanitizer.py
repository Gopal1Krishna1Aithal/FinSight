...
def _load_config() -> dict:
    ...
    defaults = {
        "salary_prefixes":    [],
        "boilerplate_phrases": [
            "PAYMENTFROMPHONEPE", "PAYMENT FROM PHONEPE",
            "POS DEBIT", "POSDEBIT",
            "AUTOPAY SI-TAD", "AUTOPAY SI-MAD",
            "AUTOPAYSI-TAD", "AUTOPAYSI-MAD",
            "COMMENTS", "IMPSTXN", "IMPS TXN",
            "ONBEHALFOF", "BENEFICIARYVERIFICATION", "IMPSTRANSACTION",
        ],
...
def _handle_salary(text: str, cfg: dict) -> str:
    ...
    return ('SALARY ' + text.strip(' -_.')).strip() if text.strip(' -_.') else 'SALARY CREDIT'
